...
func MigrateDB(db *gorm.DB) error {
	if err := db.AutoMigrate(&model.User{}, &model.RequestLog{}, &model.CkData{}, &model.CkInfo{}); err != nil {
...
func InitDB() error {
	...
	if err != nil {
		return errors.Wrap(err, "数据库连接失败, 请检查配置文件'/config.yml'中的数据库连接信息或数据库运行状态")
	}
