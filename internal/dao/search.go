...
// userData = &users[0]
func (br *customRepo[T]) FindByFieldExact(field string, value string) ([]T, error) {
	var result []T
...
// 批量精准查询-需要字段和值, 返回完美匹配的所有数据切片
func (br *customRepo[T]) FindByFieldList(field string, values []string) ([]T, error) {
	var result []T
	err := br.DB.Where(field+" IN ?", values).Find(&result).Error
	return result, err
}
